...
namespace vmf3 {
  ...
  struct SiteName {
    char data[5] = { '\0' };
    bool operator==(const SiteName& other) const noexcept { return (!std::strcmp(data, other.data)); }
    bool operator!=(const SiteName& other) const noexcept { return !(*this == other); }
    const char* const name() const noexcept { return data; }
  };
...
private:
    explicit iterator(MyDataFile* owner)
        : owner_(owner)
    {
      // prime first block
      int rc = owner_->get_next_block(current_);
...
class Vmf3 {
      ...
      double& mfw) noexcept
  {
     ...
     */
    const double bh = bc.bh_A0 + bc.bh_A1 * cd2pi + bc.bh_B1 * sd2pi + bc.bh_A2 * cd4pi + bc.bh_B2 * sd4pi;
    const double bw = bc.bw_A0 + bc.bw_A1 * cd2pi + bc.bw_B1 * sd2pi + bc.bw_A2 * cd4pi + bc.bw_B2 * sd4pi;
    const double ch = bc.ch_A0 + bc.ch_A1 * cd2pi + bc.ch_B1 * sd2pi + bc.ch_A2 * cd4pi + bc.ch_B2 * sd4pi;
    const double cw = bc.cw_A0 + bc.cw_A1 * cd2pi + bc.cw_B1 * sd2pi + bc.cw_A2 * cd4pi + bc.cw_B2 * sd4pi;

    /* calculating the hydrostatic and wet mapping factors */
    const double sel = std::sin(el);
    mfh = (1 + (ah / (1 + bh / (1 + ch)))) / (sel + (ah / (sel + bh / (sel + ch))));
    mfw = (1 + (aw / (1 + bw / (1 + cw)))) / (sel + (aw / (sel + bw / (sel + cw))));
  }

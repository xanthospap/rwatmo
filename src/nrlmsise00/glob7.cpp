...
namespace {
  ...
  double g0(double a, const double *const p) {
    return (a - 4.0 +
        (p[25] - 1.0) * (a - 4.0 +
          (std::exp(-sqrt(p[24] * p[24]) * (a - 4.0)) - 1.0) /
          std::sqrt(p[24] * p[24])));
  }
  /*    Eq. A24c */
...
double globe7(const double *const p, double tloc, double doy, double sec, double glat,
              double glon, double f107, double f107A, const double *const ap, double *plg[]) noexcept {
  /* CALCULATE G(L) FUNCTION
   * Upper Thermosphere Parameters */
  constexpr const double sr = 7.2722e-5;
  constexpr const double dr = 1.72142e-2;
  constexpr const double hr = 0.2618;

  double t[15]={0e0};
  double plg[4][9];

  /* calculate legendre polynomials */
  const double c = std::sin(glat);
  const double s = std::cos(glat);
  const double c2 = c * c;
  ...
  const double stloc  = std::sin(hr * tloc);
  const double ctloc  = std::cos(hr * tloc);
  const double s2tloc = std::sin(2.0 * hr * tloc);
  const double c2tloc = std::cos(2.0 * hr * tloc);
  const double s3tloc = std::sin(3.0 * hr * tloc);
  const double c3tloc = std::cos(3.0 * hr * tloc);
  ...
  f1 = 1.0 + (p[47] * dfa + p[19] * df + p[20] * df * df);
  f2 = 1.0 + (p[49] * dfa + p[19] * df + p[20] * df * df);
  ...
  if (p[51] != 0) {
    double exp1 = std::exp(-10800.0 * std::sqrt(p[51] * p[51]) /
        (1.0 + p[138] * (45.0 - std::abs(dso::rad2deg(glat)))));
    if (exp1 > 0.99999)
      exp1 = 0.99999;
    if (p[24] < 1.0e-4)
      p[24] = 1.0e-4;
